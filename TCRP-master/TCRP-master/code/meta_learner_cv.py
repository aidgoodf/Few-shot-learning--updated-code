...
class MetaLearner(object):
    def __init__(self, meta_dataset, fs_dataset, K, meta_lr, inner_lr, layer, hidden, tissue_num, meta_batch_size, inner_batch_size, num_updates, num_inner_updates, tissue_index_list, patience=3, num_trials=10):
        ...
        self.observed_opt = torch.optim.Adam(self.observed_tissue_model.parameters(
        ), lr=self.meta_lr, betas=(0.9, 0.99), eps=1e-05)
        self.inner_net = InnerLoop(
            self.num_inner_updates, self.inner_lr, self.feature_num, layer, hidden)

        #torch.cuda.manual_seed(args.seed)
        
        # these lines below were not commented out before,
        # I commented them out to try to keep running past the
        # assertion error that is thrown by them.
        
        self.observed_tissue_model.cpu()
        self.inner_net.cpu()

    def zero_shot_test(self, unseen_train_loader, unseen_vali_loader, unseen_test_loader):

        # builds the model for the unseen tissue
    ...
    # testing the performance of the training
        train_performance = evaluate_cv(
            unseen_tissue_model, unseen_train_loader)
    ...
    def unseen_tissue_learn(self, unseen_train_loader, unseen_test_loader):
        ...
        # builds a model for the unseen tissue
        unseen_tissue_model = mlp(self.feature_num, self.layer, self.hidden)

        # First need to copy the original meta learning model
        unseen_tissue_model.copy_weights(self.observed_tissue_model)
        unseen_tissue_model.cpu()
        # unseen_tissue_model.train()
        unseen_tissue_model.eval()

        unseen_opt = torch.optim.SGD(
            unseen_tissue_model.parameters(), lr=self.inner_lr)
...
#This causes an assertion error with cuda
        ...
        train_loss, train_corr = np.zeros(
            (self.num_updates,)), np.zeros((self.num_updates,))
        test_loss, test_corr = np.zeros(
            (self.num_updates,)), np.zeros((self.num_updates,))
        ...
        for epoch in range(self.num_updates):
            ...
            grads = []
            meta_train_loss, meta_train_corr, meta_val_loss, meta_val_corr = np.zeros((self.meta_batch_size,)), np.zeros(
                (self.meta_batch_size,)), np.zeros((self.meta_batch_size,)), np.zeros((self.meta_batch_size,))
            ...
            for i in range(self.meta_batch_size):
                ...
                observed_train_loader, observed_test_loader = get_observed_data_loader(
                    self.meta_dataset.feature, self.meta_dataset.label, self.tissue_index_list, self.K, self.inner_batch_size, self.tissue_num)

                #self.inner_net.copy_weights( self.observed_tissue_model )

                metrics, g = self.inner_net.forward(
                    observed_train_loader, observed_test_loader)
                grads.append(g)
            ...
            train_loss[epoch], train_corr[epoch], train_spearman_corr[epoch], test_loss[epoch], test_corr[
                epoch], test_spearman_corr[epoch], _, _ = self.unseen_tissue_learn(unseen_train_loader, unseen_test_loader)
