...
def evaluate_new_PDX(net, loader, train_flag, weights=None):
	...
	test_predict = torch.zeros(0,0).cpu()
	test_label = torch.zeros(0,0).cpu()
	cat_test_label = torch.zeros(0,0).cpu()
	total_loss = 0
...
def evaluate_new(net, loader, train_flag, weights=None):
	...
	test_predict = torch.zeros(0).cpu()
	test_label = torch.zeros(0).cpu()
	total_loss = 0
